...
This repo is my personal interest project data pipeline and dashboard for the NYT Besteller Lists both current and historical. This repo both 1) collects data on the NYT Bestseller Lists and uploads it to an internal database, and 2) visualizes the current and historical data -- with book rankings and info -- in a Streamlit app.

## ETL Pipeline

The sub-folder `src/etl/` as well as the folder `dags/` contain code that __collects NYT Bestseller List data from their public API, transforms it, and loads it to a cloud database__ (Azure SQL). The pipeline runs on a weekly cadence to coincide with the NYT's update schedule for their Bestseller Lists, and is executed through a scheduled GitHub Actions workflow that uses a temporary Airflow instance.

The repo also includes a separate workflow that executes a __backfill of historical data__, executing a specialized implementation of the pipeline over a list of dates to use in iterative API calls (see sub-folder `src/backfill/`). It runs only via manual dispatch and is programmed to dynamically infer the dates needed for the backfill by scanning the database for the earliest date and pairing this date with the known API date floor (June 4, 2008) to construct the date range. The workflow can also accept user-provided dates.
